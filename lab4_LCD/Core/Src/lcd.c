		...
		uint8_t                 device_address)
{
	...
	TextLCD_SendByte(hlcd, 0x0F, 0xB); //Display on, Cursor On, Blink on
}
...
void TextLCD_Position	(TextLCDType * hlcd, int col, int row)
{
	...
	{
		index = 0x40 + col;		//rad 2 börjar på 0x40
	}
	else
	{
		return;
	...
	TextLCD_SetDDRAMAdr(hlcd, index);
}
